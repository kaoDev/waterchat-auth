...
const stateHasRegisteredUser = ({ users }: State) => (userId: string) => users.some(({ userId: id }) => userId === id);

const authorizeUserRegisteredEvent = ({ users }: State) => ({ userId, identifiers }: UserRegistered) => users.every(({ userId: id, identifiers: oldIdentifiers }) => {
    const idsUnEqual = userId !== id;

    return idsUnEqual && !identifiers.some(i => oldIdentifiers.some(oldI => oldI.provider === i.provider && oldI.id === i.id));
});
