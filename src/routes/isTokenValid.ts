...
export const POST = async (req: IncomingMessage, res: ServerResponse) => {
  try {
    ...
    if (!accessToken || !provider) {
      ...
      userState.take(1).subscribe(async state => {
        console.log('GOT STATE', state)
        const user = state.users.find(u => {
          return u.identifiers.some(
            i => i.accessToken === accessToken && i.provider === provider,
          )
        })
        ...
        if (user === undefined) {
          ...
          const identifier = user.identifiers.find(
            i => i.accessToken === accessToken && i.provider === provider,
          ) as GitHubUserIdentifier
          if (
            identifier !== undefined &&
            timestampValid(identifier.timestamp)
          ) {
            httpOK(res)
          } else {
